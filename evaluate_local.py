...
def run_evaluation(sample_idx=None, save_path="hackathon_final_plot.png", weights=WEIGHTS):
    ...
    ax_bin.imshow(np.flipud(pred_bin), cmap="gray", extent=BEV_EXTENT, origin="lower")
    add_ego_marker(ax_bin); style_ax(ax_bin, f"Binary Prediction (IoU={iou:.1f}%)")
    ...
    plt.suptitle(f"MAHE Mobility BEV Evaluation | Sample {sample_idx} | IoU {iou:.2f}%", color="white", fontsize=13, y=0.97)
