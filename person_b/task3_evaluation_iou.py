...
from config import (X_MIN, X_MAX, Y_MIN, Y_MAX,
                    RESOLUTION, GRID_W, GRID_H, OCC_THRESHOLD)
...
def compute_occupancy_iou(
    ...
    TP = int(np.logical_and( pred_bin,  gt_bin).sum())   # hit
    FP = int(np.logical_and( pred_bin, ~gt_bin).sum())   # false alarm
    FN = int(np.logical_and(~pred_bin,  gt_bin).sum())   # miss
    TN = int(np.logical_and(~pred_bin, ~gt_bin).sum())   # correct reject
...
def _weighted_iou(pred_bin: np.ndarray, gt_bin: np.ndarray) -> float:
    ...
    """
    # Build weight mask (same formula as Task 2)
    col_c = X_MIN + (np.arange(GRID_W) + 0.5) * RESOLUTION
    row_c = Y_MIN + (np.arange(GRID_H) + 0.5) * RESOLUTION
    X, Y  = np.meshgrid(col_c, row_c)
    W     = 1.0 / (np.sqrt(X**2 + Y**2) + 1.0)   # (H, W)
...
def visualise_error_map(
    ...
    im = axes[1].imshow(flip(pred_probs), cmap="hot", origin="lower",
                        extent=extent, vmin=0, vmax=1)
    axes[1].set_title("Prediction (probability)")
    plt.colorbar(im, ax=axes[1], fraction=0.046)

    # Panel 2: binarised prediction
    axes[2].imshow(flip(pred_bin.astype(float)), cmap="gray",
                   origin="lower", extent=extent)
    axes[2].set_title(f"Prediction (threshold={threshold})")

    # Panel 3: error map
    axes[3].imshow(flip(error_map), cmap=cmap_err, origin="lower",
                   extent=extent, vmin=0, vmax=3)
...
if __name__ == "__main__":
    ...
    print(f"  Weighted IoU gap   : {m_good.iou_weighted - m_bad.iou_weighted:.4f}  ← should be larger")
