...
from models.resnet_extractor import ResNetFeatureExtractor   # 2-D backbone
from geometry.lss_core import (                              # LSS geometry engine
...
class LiftHead(nn.Module):
    ...
    def __init__(self, in_channels: int, depth_cfg: DepthConfig,
    ...
    def forward(self, feat: torch.Tensor) -> torch.Tensor:
        ...
        feat_up = F.interpolate(feat, size=(self.img_h, self.img_w),
...
class BEVModel(nn.Module):
    ...
    def __init__(
        ...
        self.lift_head = LiftHead(in_channels=out_channels,
                                  depth_cfg=depth_cfg,
                                  cam_cfg=cam_cfg)
...
class NuScenesFrontCameraDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        cal = self.nusc.get('calibrated_sensor',
                            cam_front_data['calibrated_sensor_token'])
        intrinsic   = torch.tensor(cal['camera_intrinsic'], dtype=torch.float32)
        translation = torch.tensor(cal['translation'],      dtype=torch.float32)
        rotation    = torch.tensor(cal['rotation'],         dtype=torch.float32)
...
def train_pipeline(
    ...
    # Model
    model = BEVModel(out_channels=out_channels,
                     cam_cfg=cam_cfg,
                     bev_cfg=bev_cfg,
                     depth_cfg=depth_cfg).to(device)
